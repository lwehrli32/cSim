...
void init_cache() {
	...
	// goes through the sets and creates E lines
	for (int i = 0; i < S; i++){		
		// now go through the set and create the lines
		...
		// go though the lines and set the valid and tag bits
		for (int j = 0; j < E; j++){
			
			// sets the tag and valid bits
			cache[i][j].valid = '0';
			cache[i][j].tag = (mem_addr_t)0;
			cache[i][j].LRU = 0;
		}
	}         
}
...
void access_data(mem_addr_t addr) {
	...
	for (int j = 0; j < E; j++){
		if (cache[set][j].tag == tag && cache[set][j].valid == '1'){
			...
			// hit
			++hit_cnt;
			cache[set][j].LRU = highestCounter + 1;
			++highestCounter;
			return;
		}		
	}
	...
	// checks for evictions
	int smallestCounter = INT_MAX;
	mem_addr_t smallestTag = (mem_addr_t) 0;
	for (int j = 0; j < E; j++){
		if (cache[set][j].LRU < smallestCounter && cache[set][j].valid == '1'){
			
			// this is the tag that needs to be evicted
			smallestTag = cache[set][j].tag;
			smallestCounter = cache[set][j].LRU;
		}
	}	
	
	// finds the tag that needs to be evicted
	for (int j = 0; j < E; j++){
		if (cache[set][j].tag == smallestTag){

			// goes through and evicts the tag
			cache[set][j].tag = tag;
			cache[set][j].LRU = highestCounter + 1;
			++highestCounter;
			++evict_cnt;
...
void replay_trace(char* trace_fn) {           
    ...
    while (fgets(buf, 1000, trace_fp) != NULL) {
        if (buf[1] == 'S' || buf[1] == 'L' || buf[1] == 'M') {
			...
			// calls access_data again if buf[1] is M
			if (buf[1] == 'M'){
				access_data(addr);
			}
